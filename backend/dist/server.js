...
Format as JSON array, like: ["point1", "point2", "point3"]`;
        ...
        if (process.env.SERPER_API_KEY) {
            const verifiedClaims = await Promise.all(claims.slice(0, 3).map(async (claim) => {
                try {
                    const response = await axios.post('https://google.serper.dev/search', { q: claim, num: 5 }, {
                        headers: {
...
app.post('/api/chat', verifyAuthToken, async (req, res) => {
    try {
        const { message, userId, articleContext } = req.body;
...
// ==================== REPORT INACCURACY ====================
app.post('/api/report-inaccuracy', verifyAuthToken, async (req, res) => {
    try {
        const { text, report, userId, reliabilityScore } = req.body;
...
// ==================== USER HISTORY ====================
app.get('/api/user/history', verifyAuthToken, async (req, res) => {
    try {
